...
JSON=`printf '{"%s":"%s","%s":"%s","%s":"%s","%s":"%s","%s":"%s","%s":"%s","%s":"%s","%s":"%s","%s":"%s","%s":%s,"%s":%s,"%s":%s,"%s":%s,"%s":%s,"%s":%s}' "host" "$HOST" "ip" "$ETH0IP" "ipv6" "$ETH0IP6" "wip" "$WLANIP" "wipv6" "$WLANIP6" "cpu" "$CPUNAME" "arch" "$CPUARCH" "temperature" "$CPUTEMP" "model" "$MODEL" "usage" "$CPUUSAGE" "core" "$CPUCORE" "mem" "$MEMT" "memf" "$MEMF" "freq" "$CPUFREQ" "scriptver" "$SCRIPTVER"`
#fi
echo $JSON
