...
curl -X POST --data "{\"host\":\"$HOST\",\"ip\":\"$ETH0_IP\",\"temperature\":\"$CPU_TEMP\",\"cpu\":\"$CPU\",\"core\":\"$CPU_CORE\",\"arch\":\"$CPU_ARCH\",\"model\":\"$MODEL\",\"mem\":\"$MEM\",\"memf\":\"$MEMF\"}" http://test.qiuhao.online/rpi_status/
